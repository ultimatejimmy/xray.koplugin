...
describe("xray_utils", function()
    ...
    describe("Slovak/Czech declension helpers", function()
        it("lowercases Slovak-only capitals", function()
            assert.are.equal("ľudovít", utils:utf8Lower("Ľudovít"))
            assert.are.equal("ĺ ŕ", utils:utf8Lower("Ĺ Ŕ"))
        end)

        it("recognizes declined forms of names", function()
            assert.is_true(utils:isInflectedForm("peter", "petra"))
            assert.is_true(utils:isInflectedForm("peter", "petrovi"))
            assert.is_true(utils:isInflectedForm("pavol", "pavlom"))
            assert.is_true(utils:isInflectedForm("janko", "janka"))
            assert.is_true(utils:isInflectedForm("bratislava", "bratislave"))
            assert.is_true(utils:isInflectedForm("mária", "márie"))
            assert.is_true(utils:isInflectedForm("tomáš", "tomášovi"))
            assert.is_true(utils:isInflectedForm("harry", "harryho"))
            -- Czech
            assert.is_true(utils:isInflectedForm("petr", "petrovi"))
            assert.is_true(utils:isInflectedForm("karel", "karla"))
            assert.is_true(utils:isInflectedForm("olomouc", "olomouci"))
            assert.is_true(utils:isInflectedForm("ostrava", "ostravě"))
        end)

        it("rejects unrelated words and too-short names", function()
            assert.is_false(utils:isInflectedForm("peter", "pavol"))
            assert.is_false(utils:isInflectedForm("peter", "petržalka"))
            assert.is_false(utils:isInflectedForm("eva", "evy"))
            assert.is_false(utils:isInflectedForm("bratislava", "bratislavskejšieho"))
        end)

        it("detects Slovak from the X-Ray language or the book language", function()
            assert.is_true(utils:usesInflection({ ai_helper = { current_language = "sk" } }))
            assert.is_true(utils:usesInflection({ ai_helper = { current_language = "cs" } }))
            assert.is_true(utils:usesInflection({ ui = { document = { getProps = function() return { language = "sk-SK" } end } } }))
            assert.is_true(utils:usesInflection({ ui = { document = { getProps = function() return { language = "cs" } end } } }))
            assert.is_false(utils:usesInflection({ ai_helper = { current_language = "en" },
                ui = { document = { getProps = function() return { language = "en-US" } end } } }))
            assert.is_false(utils:usesInflection(nil))
        end)
    end)
end)
